...
int main()
{
	//system("color F0");
	//加载测试数据
	Mat data = imread("D:/images/所有数据按行排列结果.png", IMREAD_ANYDEPTH);
	Mat labels = imread("D:/images/标签.png", IMREAD_ANYDEPTH);
	...
	DTmodel->setUseSurrogates(false);  //是否建立替代分裂点
	DTmodel->setMinSampleCount(2);  	//节点最小样本数量
	DTmodel->setUse1SERule(false);  //是否严格修剪
	DTmodel->setTruncatePrunedTree(false);  //分支是否完全移除

	Ptr<ml::TrainData> trainData = ml::TrainData::create(data, ml::ROW_SAMPLE, labels);
	DTmodel->train(trainData);
	DTmodel->save("D:/images/DTrees_model.yml");
	...
	Mat testImg1 = imread("D:/images/handWrite01.png", IMREAD_GRAYSCALE);
	Mat testImg2 = imread("D:/images/handWrite02.png", IMREAD_GRAYSCALE);
